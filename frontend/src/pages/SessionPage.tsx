...
interface WorkoutRecordForm {
    ...
    notes?: string;
    workoutId?: number;
    activityRecords: ActivityRecordForm[];
}
...
const SessionPage: React.FC<SessionFormProps> = ({
    ...
    useEffect(() => {
        ...
        if (!selectedWorkout || !selectedWorkout.workoutActivities) return;

        // Map and sort all activities from the workoutActivities array
        const sortedActivities = selectedWorkout.workoutActivities.sort((a, b) => a.order - b.order);

        const newActivityRecords: ActivityRecordForm[] = sortedActivities.map(wa => {
            const activity = wa.activity as Activity;
            const exerciseRecords = (activity.group || []).map(ae => ({
                notes: "",
                name: ae.exercise.name,
                exerciseId: ae.exercise.id,
                load: ae.exercise.load || undefined,
                reps: ae.exercise.reps || undefined,
                sets: ae.exercise.sets || undefined,
                time: ae.exercise.time || undefined,
            }));
    ...
    // Use useMemo to group activities by phase for rendering
    const groupedActivities = useMemo(() => {
        const groups: Record<string, ActivityRecordForm[]> = {
            warmup: [],
            work: [],
            cooldown: []
        };
        workoutRecord.activityRecords.forEach(ar => {
            groups[ar.phase].push(ar);
        });
        return groups;
    ...
    // Handler for a single, combined activityRecords array
    const handleActivityRecordChange = (
        activityIdx: number,
        field: keyof ActivityRecordForm,
        value: any
    ) => {
        const updated = [...workoutRecord.activityRecords];
        updated[activityIdx] = { ...updated[activityIdx], [field]: value };
        setWorkoutRecord({ ...workoutRecord, activityRecords: updated });
    };
    ...
    const handleExerciseRecordChange = (
        ...
        const updatedActivities = [...workoutRecord.activityRecords];
        ...
        setWorkoutRecord({ ...workoutRecord, activityRecords: updatedActivities });
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const activityRecords = workoutRecord.activityRecords.map(ar => ({
            notes: ar.notes,
            phase: ar.phase,
    ...
    const renderActivitySection = (phase: keyof typeof groupedActivities) => (
        <Stack spacing={2}>
            {groupedActivities[phase].map((ar) => {
                const globalIndex = workoutRecord.activityRecords.findIndex(item => item.name === ar.name && item.phase === phase);
                return (
                    <Paper key={ar.id} sx={{ p: 2 }}>
                        ...
                        <TextField
                            ...
                            multiline
                            value={ar.notes || ""}
                            onChange={e => handleActivityRecordChange(globalIndex, "notes", e.target.value)}
                            fullWidth
                            sx={{ mb: 1 }}
                        />
                        ...
                        <Stack spacing={1}>
                            {ar.exerciseRecords.map((er, eidx) => (
                                <Paper key={er.id} sx={{ p: 1, background: "#f9f9f9" }}>
                                    <Typography variant="body2">{`Exercise: ${er.name}`}</Typography>
                                    <TextField
                                        label="Load"
                                        type="number"
                                        value={er.load || ""}
                                        onChange={e => handleExerciseRecordChange(globalIndex, eidx, "load", Number(e.target.value))}
                                        sx={{ mr: 1, width: 120 }}
                                    />
                                    <TextField
                                        label="time (sec)"
                                        type="number"
                                        value={er.time || ""}
                                        onChange={e => handleExerciseRecordChange(globalIndex, eidx, "time", Number(e.target.value))}
                                        sx={{ mr: 1, width: 150 }}
                                    />
                                    <TextField
                                        label="Reps"
                                        type="number"
                                        value={er.reps || ""}
                                        onChange={e => handleExerciseRecordChange(globalIndex, eidx, "reps", Number(e.target.value))}
                                        sx={{ mr: 1, width: 100 }}
                                    />
                                    <TextField
                                        label="Sets"
                                        type="number"
                                        value={er.sets || ""}
                                        onChange={e => handleExerciseRecordChange(globalIndex, eidx, "sets", Number(e.target.value))}
                                        sx={{ mr: 1, width: 100 }}
                                    />
                                    <TextField
                                        label="Notes"
                                        value={er.notes || ""}
                                        onChange={e => handleExerciseRecordChange(globalIndex, eidx, "notes", e.target.value)}
                                        sx={{ width: 200 }}
                                    />
                                </Paper>
                            ))}
    ...
    return (
        <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 900, mx: "auto", p: 2 }}>
            ...
            <Stack spacing={2}>
                ...
                {renderActivitySection("warmup")}
                <Typography variant="subtitle1">Work Records</Typography>
                {renderActivitySection("work")}
                <Typography variant="subtitle1">Cooldown Records</Typography>
                {renderActivitySection("cooldown")}
                <Divider sx={{ my: 2 }} />
